  ...
  const deleteworkstatus = async (guideId, workStatusid, clientid) => {
    try{
      ...
      if(confirmation){
        ...
        await axios.put(`http://13.200.240.28:5000/api/client/editClient/${clientid}`,{workerid:'', assignStatus:'NOT ASSIGNED'});
          ...
          <CTableBody>
            {guides.map((guide, index) => (
              <CTableRow key={guide._id}>
                ...
                <CTableDataCell  style={{ fontSize: '0.870rem' }}>
                <CButton color='info' onClick={() => handleworkstatus(guide._id)} size='sm' className="p-2">view Status</CButton>
                </CTableDataCell>
                {/* <CTableDataCell  style={{ fontSize: '0.870rem' }}>
