...
const HomecareManagement = () => {
  ...
  const handleAddGuide = async (event) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/guide/register', formData ,{
        headers: {
  ...
  const handleEditGuide = async (event) => {
    ...
    try {
      await axios.put(`http://localhost:5000/api/guide/editguide/${_id}`, formData);
  ...
  const  handleworkstatus = async (id) => {
    try{
      const response = await axios.get(`http://localhost:5000/api/guide/getguide/${id}`);
  ...
  const deleteworkstatus = async (guideId, workStatusid, clientid) => {
    try{
      ...
      if(confirmation){
        ...
        const response = await axios.get(`http://localhost:5000/api/guide/getguide/${guideId}`);
        ...
        await axios.put(`http://localhost:5000/api/guide/updateworker/${guideId}`, {modelclientid:updatedmodelclientid});
      
        //removes workerid from clientdata
        await axios.put(`http://localhost:5000/api/client/editClient/${clientid}`,{workerid:'', assignStatus:'NOT ASSIGNED', assigned:'ASSIGN'});
        await axios.put(`http://localhost:5000/api/guide/deleteworkstatus/${guideId}`,{workStatus:updatedguidedata});
      }
            ...
            {guides.map((guide, index) => (
              <CTableRow key={guide._id}>
                ...
                <CTableDataCell style={{ fontSize: '0.870rem' }}>
                  {guide.image && <img src={`http://localhost:5000${guide.image}`} alt={guide.name} style={{ width: '100px' }} />}
                </CTableDataCell>
